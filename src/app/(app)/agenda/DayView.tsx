      ...
      {rows.map((row, i) => {
        ...
        if (a.kind === "block") {
          const endTime = minutesToTimeString(
            localMinutesInTz(new Date(a.start_at), timezone) + a.duration_minutes,
          );
          const affected = a.professional?.name ?? "Todos";
          return (
            <button
              key={a.id}
              type="button"
              onClick={() => onOpenBlock(a)}
              className="w-full flex items-center gap-3 px-5 py-3.5 border-b border-divider text-left cursor-pointer bg-neutral-200"
            >
              <div className="w-12 flex-none font-heading font-extrabold text-sm opacity-60">{time}</div>
              <div className="flex-1 min-w-0 flex flex-col gap-0.5">
                <span className="text-sm opacity-70">
                  Bloqueado {time}–{endTime}
                  {a.reason ? ` · ${a.reason}` : ""}
                </span>
                <span className="text-xs opacity-50">{affected}</span>
              </div>
            </button>
          );
        }
